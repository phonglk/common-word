...
const pre = function pre(a) {
  return CLASS_PREFIX + a;
};

const CLASS_WORD = exports.CLASS_WORD = pre('word');
const CLASS_UNKNOWN_WORD = exports.CLASS_UNKNOWN_WORD = pre('word--unknown');
const CLASS_KNOWN_WORD = exports.CLASS_KNOWN_WORD = pre('word--known');

const CLASS_TOOL_ADD_TO_KNOWN_LIST = exports.CLASS_TOOL_ADD_TO_KNOWN_LIST = pre('tool--known');
const CLASS_TOOLBAR_MOUNT_DIV = exports.CLASS_TOOLBAR_MOUNT_DIV = pre('tool--div-mount');
const CLASS_PROGRESS_TO_SHOW_TOOLBAR = exports.CLASS_PROGRESS_TO_SHOW_TOOLBAR = pre('progress--toolbar-enable');
const CLASS_SIDEBAR_WRAP = exports.CLASS_SIDEBAR_WRAP = pre('sidebar--wrapper');
const CLASS_SIDEBAR_WORDLIST = exports.CLASS_SIDEBAR_WORDLIST = pre('sidebar--wordlist');
const CLASS_SIDEBAR_KNOWN_WORD = exports.CLASS_SIDEBAR_KNOWN_WORD = pre('sidebar--known-word');
const CLASS_SIDEBAR_TOOLBAR = exports.CLASS_SIDEBAR_TOOLBAR = pre('sidebar--toolbar');
...
function mouseenterHandler(word, replacementNode, event) {
  ...
  if (!targetNode.classList.contains(_style.CLASS_UNKNOWN_WORD)) return;
  ...
  targetNode.attributes.enterTO = setTimeout(_actions.showToolbar.bind(null, { word, targetNode, event, wordListMap }), _style.THRESHOLD_TOOLBAR_SHOW);
}
...
let hideWord = exports.hideWord = function () {
  var _ref2 = _asyncToGenerator(function* ({ word, targetNode }) {
    ...
    [...document.querySelectorAll(`.${_style.CLASS_WORD}[${_style.PROP_WORD}="${word}"]`)].forEach(function (node) {
      node.classList.remove(_style.CLASS_UNKNOWN_WORD);
      node.classList.add(_style.CLASS_KNOWN_WORD);
    });
...
function showProgressBar({ targetNode }) {
  let progressNode = targetNode.querySelector(`.${_style.CLASS_PROGRESS_TO_SHOW_TOOLBAR}`);
  if (!progressNode) {
    progressNode = document.createElement('div');
    progressNode.classList.add(_style.CLASS_PROGRESS_TO_SHOW_TOOLBAR);
    targetNode.append(progressNode);
  }
}

function removeProgressBar({ targetNode }) {
  const progressNode = targetNode.querySelector(`.${_style.CLASS_PROGRESS_TO_SHOW_TOOLBAR}`);
...
class SideBar extends _react2.default.Component {
  ...
  render() {
    ...
    return _react2.default.createElement(
      'div',
      { className: _style.CLASS_SIDEBAR_WRAP },
      _react2.default.createElement(
        'div',
        { className: _style.CLASS_SIDEBAR_TOOLBAR },
        'Sort \xA0',
        _react2.default.createElement(
          'a',
      ...
      _react2.default.createElement(
        'div',
        { className: _style.CLASS_SIDEBAR_WORDLIST },
        filteredList.map(function (w) {
          return _react2.default.createElement(
            'div',
            { key: w.rank, className: w.isKnown ? _style.CLASS_SIDEBAR_KNOWN_WORD : '' },
            w.word,
            ' [',
            w.rank,
            '] (',
