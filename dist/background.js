...
var _defaultStorage = __webpack_require__(20);

function _asyncToGenerator(fn) { return function () { var gen = fn.apply(this, arguments); return new Promise(function (resolve, reject) { function step(key, arg) { try { var info = gen[key](arg); var value = info.value; } catch (error) { reject(error); return; } if (info.done) { resolve(value); } else { return Promise.resolve(value).then(function (value) { step("next", value); }, function (err) { step("throw", err); }); } } return step("next"); }); }; }

exports.default = function () {
  var _ref = _asyncToGenerator(function* () {
    const hasSync = yield (0, _storage.getSync)('_hasSyncStorage');
    if (hasSync !== true) {
      (0, _log.info)('This extension looks like first time run');
      yield (0, _storage._set)(_defaultStorage.sync);
      (0, _log.info)('Configured for the first run');
    } else {
      (0, _log.info)('Extension configuration looks ok');
    }
  });
...
exports.default = function () {
  var _ref = _asyncToGenerator(function* (url, option = {}) {
    return new Promise(function (resolve, reject) {
      const fetchOption = _extends({}, option, {
        method: 'GET',
        headers: {},
        mode: 'cors',
        type: 'json',
      ...
      window.fetch(url, fetchOption).then(function (response) {
...
ParseException = __webpack_require__(11);
...
const sync = exports.sync = {
  wordLists: [{
    id: '5kmcw',
    name: '5000 Most common words',
    yaml: 'https://raw.githubusercontent.com/phonglk/most-common-words-repo/master/5000.yaml'
  }],
  knownList: {
    '5kmcw': [/* {word, date, sentence} */]
  },
  selectedWordList: '5kmcw',
  _hasSyncStorage: true
};
...
Unescaper = __webpack_require__(28);

Escaper = __webpack_require__(25);

Utils = __webpack_require__(6);

ParseException = __webpack_require__(11);

DumpException = __webpack_require__(26);
...
let getSelectedWordList = function () {
  var _ref = _asyncToGenerator(function* () {
    const selectedListId = yield (0, _storage.getSync)('selectedWordList');
    const cachedWordList = yield (0, _storage.getLocal)('currentWordList');
    if (cachedWordList && cachedWordList.id === selectedListId) {
      (0, _log.info)(`wordList ${cachedWordList.id} is cached`);
    ...
    const wordListArray = yield (0, _storage.getSync)('wordLists');
    const selectedList = wordListArray.find(function (list) {
      return list.id === selectedListId;
    });
    (0, _log.info)(`request wordList from ${selectedList.yaml}`);
    const response = yield (0, _request2.default)(selectedList.yaml, { responseAs: 'text' });
    const list = yamlParser.parse(response);
    newCachedWordList.list = list;
    yield (0, _storage.setLocal)('currentWordList', newCachedWordList);
    (0, _log.info)(`wordList ${selectedListId} is newly cached`);
    return newCachedWordList;
  });

  return function getSelectedWordList() {
    return _ref.apply(this, arguments);
  };
}();
...
let main = function () {
  var _ref2 = _asyncToGenerator(function* () {
    yield (0, _preliminaryCheck2.default)();
    const wordList = yield getSelectedWordList();
...
var _preliminaryCheck = __webpack_require__(12);

var _preliminaryCheck2 = _interopRequireDefault(_preliminaryCheck);
...
const yamlParser = new _Parser2.default();

function bindBrowserAction({ wordList }) {
  chrome.browserAction.onClicked.addListener(function (tab) {
    (0, _communication.sendMessageToTab)(tab.id, {
      event: _event.EVENT_BROWSER_ACTION_CLICK,
      wordList
    });
  });
}
...
function accommodateInPageConnection() {
  ...
  const getSenderId = function getSenderId(sender) {
    return `${sender.tab.id}${sender.tab.windowId}`;
  };
  const getConnectionById = function getConnectionById(id) {
  ...
  (0, _communication.addMessageListener)(function (payload, sender, sendResponse) {
    if (payload.type === _communication.INIT_IN_PAGE_CONNECTION) {
